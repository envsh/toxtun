...
type EchoPing interface {
	Ping(string, int) (time.Duration, error)
}

func Ping2(addr string, timeout int) (time.Duration, error) {
	btime := time.Now()
	...
	return etime.Sub(btime), errors.New("timeout/errperm")
}

func Ping1(addr string, timeout int) (time.Duration, error) {
